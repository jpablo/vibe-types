...
|---------|-----------------|
| **Generics / TypeVar** [-> catalog/07](07-generics-typevar.md) | A `Callable` can use `TypeVar` to express generic callbacks: `Callable[[T], T]` preserves the input type through the return. |
| **ParamSpec** [-> catalog/08](08-paramspec-typevar-tuple.md) | `ParamSpec` captures an entire parameter list, letting decorators preserve the wrapped function's full signature — something `Callable` alone cannot do. |
| **Protocol** [-> catalog/09](09-protocol-structural-subtyping.md) | A Protocol with `__call__` replaces `Callable` when the callback needs keyword arguments, optional parameters, or overloaded signatures. |
...
- [-> UC-07](../usecases/07-api-contracts-callable.md) — Callback-driven architectures where functions are passed as configuration.
- [-> UC-09](../usecases/09-configuration-builder.md) — Plugin systems that accept callable hooks with typed signatures.
- [-> UC-11](../usecases/11-decorator-typing.md) — Event handlers and signal/slot patterns with typed callbacks.
