...
|---------|-----------------|
| **Dataclasses** [-> catalog/06](06-dataclasses-typing.md) | `ClassVar` fields are excluded from the generated `__init__`, `__repr__`, and comparison methods. `Final` fields work but require `default` or `default_factory` since they cannot be reassigned. |
| **Protocol** [-> catalog/09](09-protocol-structural-subtyping.md) | A Protocol can declare `ClassVar` members to require class-level attributes. `Final` is not meaningful in Protocol definitions since Protocols describe structural shape, not implementation. |
| **Enum** [-> catalog/05](05-enums-typing.md) | Enum members are implicitly final. Adding explicit `Final` annotations to enum members is redundant but harmless. |
| **Annotated** [-> catalog/15](15-annotated-metadata.md) | `Final` and `ClassVar` can appear inside `Annotated` as the base type: `Annotated[Final[int], ...]` is valid in some contexts, though checker support varies. |
...
- [-> UC-06](../usecases/06-immutability-finality.md) — Configuration objects where `Final` ensures immutable settings and `ClassVar` manages shared defaults.
