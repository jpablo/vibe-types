...
- [→ UC-02](../usecases/02-ownership-safe-apis.md) — Lifetime annotations encode borrowing contracts that callers cannot violate.
- [→ UC-05](../usecases/05-compile-time-concurrency-constraints.md) — Thread-safety often requires `'static` bounds so data sent across threads cannot dangle.
