...
- [-> UC-03](../usecases/03-generic-capability-constraints.md) — Using trait bounds to constrain generic parameters so APIs only accept types with the right capabilities.
- [-> UC-04](../usecases/04-extensible-polymorphic-interfaces.md) — Designing extensible interfaces where new types can be added without modifying existing code.
- [-> UC-05](../usecases/05-compile-time-concurrency-constraints.md) — Marker traits `Send` and `Sync` enforce thread-safety at compile time.
- [-> UC-07](../usecases/07-trait-impl-failure-diagnostics.md) — Reading and resolving trait-related compiler errors in real-world code.
