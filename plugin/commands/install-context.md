...
1. **Choose the language.** If `$ARGUMENTS` specifies a language, use it. Otherwise:
   - First, detect the project language by checking for signature files: `*.py` or `pyproject.toml` → Python, `Cargo.toml` or `*.rs` → Rust, `lakefile.lean` or `*.lean` → Lean 4, `build.sbt` or `build.sc` or `*.scala` → Scala 3, `tsconfig.json` or `*.ts`/`*.tsx` → TypeScript.
...
4. **Append the snippet** to the end of the file (create it if it doesn't exist). Read the snippet from the canonical source file at `${CLAUDE_PLUGIN_ROOT}/skills/<lang>/quick-index.md` (where `<lang>` is `python`, `rust`, `lean`, `scala3`, or `typescript`). Wrap the content in marker comments `<!-- vibe-types:<lang> -->` / `<!-- /vibe-types:<lang> -->`. Include a blank line before the marker.
