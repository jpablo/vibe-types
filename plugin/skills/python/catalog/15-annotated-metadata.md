...
|---------|-----------------|
| **Basic annotations** [-> catalog/01](01-basic-annotations-none.md) | `Annotated` wraps any base type. `Annotated[int \| None, ...]` works as expected — the checker sees `int \| None`. |
| **TypedDict** [-> catalog/03](03-typeddict.md) | TypedDict values can be `Annotated` to attach per-field metadata: `class User(TypedDict): age: Annotated[int, Gt(0)]`. |
| **Dataclasses** [-> catalog/06](06-dataclasses-typing.md) | Dataclass fields with `Annotated` types let runtime validators (Pydantic dataclasses, beartype) enforce constraints on construction. |
| **Unpack / kwargs** [-> catalog/19](19-unpack-kwargs-typing.md) | `Annotated` can appear inside `Unpack[TypedDict]` field types, attaching metadata to individual keyword arguments. |
...
- [-> UC-02](../usecases/02-domain-modeling.md) — API boundary validation where Annotated bridges static types and runtime enforcement.
- [-> UC-09](../usecases/09-configuration-builder.md) — Pydantic/FastAPI models that use Annotated for field constraints and OpenAPI schema generation.
- [-> UC-10](../usecases/10-typed-dictionaries-records.md) — Domain modeling where Annotated carries business rules alongside types.
