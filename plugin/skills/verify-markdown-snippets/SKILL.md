...
- `scripts/verify_lean.py` — same contract for Lean 4: writes the snippet into `projects/lean-project/snippet_tmp/` and runs `lake env lean` against the pinned toolchain. No wrapping (top-level Lean commands are valid as-is); parses `file:line:col: error|warning:` diagnostics, keeping only those from the snippet file. Resolves `lake` from `~/.elan/bin` when it isn't on PATH.
