...
- [→ catalog/06](../catalog/06-propositions-as-types.md) — Propositions as types encode invariants as `Prop`; construction requires proof terms.
- [→ catalog/02](../catalog/02-dependent-types.md) — Dependent types let types carry value-level constraints.
- [→ catalog/14](../catalog/14-subtypes-refinements.md) — Subtypes bundle values with proofs of predicates.
- [→ catalog/13](../catalog/13-proof-automation.md) — Tactics (`omega`, `simp`, `decide`) discharge proof obligations automatically.
