...
Lean's `inductive` types correspond to Scala 3's `enum`. Scala's GADTs (`case IntLit(v: Int) extends Expr[Int]`) are Lean's *indexed families* (`| intLit : Int → Expr Int`). The key upgrade: Lean can index by values, not just types — `Vec α n` tracks length `n : Nat` at the type level, something Scala's GADTs cannot express directly.
