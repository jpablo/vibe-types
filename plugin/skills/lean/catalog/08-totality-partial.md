...
- [→ UC-03](../usecases/03-totality-exhaustiveness.md) — Totality ensures every function handles all inputs.
- [→ UC-07](../usecases/07-safe-recursion-termination.md) — Patterns for making recursive functions total.
- [→ UC-10](../usecases/10-interop-escape-hatches.md) — `partial` as an escape hatch alongside `sorry` and `unsafe`.
