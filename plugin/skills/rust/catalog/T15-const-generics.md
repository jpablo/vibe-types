...
- [-> UC-08](../usecases/UC18-type-arithmetic.md) — Encoding value-level invariants (lengths, capacities, dimensions) into types so the compiler can enforce them.
- [-> UC-04](../usecases/UC14-extensibility.md) — Const-generic traits enable extensible interfaces parameterized by compile-time values (e.g., buffer sizes, protocol versions).
