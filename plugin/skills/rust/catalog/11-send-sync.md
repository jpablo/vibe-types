...
- [-> UC-05](../usecases/05-compile-time-concurrency-constraints.md) — Send and Sync are the foundation of compile-time concurrency safety, preventing data races at the type level.
- [-> UC-02](../usecases/02-ownership-safe-apis.md) — API boundaries that accept `T: Send` communicate thread-transfer intent in the signature.
