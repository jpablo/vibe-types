...
9. **`Object.freeze` is not the same as `readonly`** — `Object.freeze(obj)` enforces immutability at runtime, and its return type is `Readonly<T>`, so `const frozen = Object.freeze(obj)` does give you readonly properties. What it cannot do is retype the original binding: a bare `Object.freeze(obj)` statement leaves `obj`'s own type mutable, so `obj.field = x` still compiles and fails only at runtime.
