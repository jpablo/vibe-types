...
- **Union types.** Nullability is encoded as `T | Null`, reusing Scala 3's first-class union type mechanism. All union-type rules (pattern matching, subtyping) apply directly. [-> UC-01](../usecases/UC01-invalid-states.md)
...
- **Pattern matching.** Match cases like `case _: String =>` narrow a `String | Null` scrutinee to `String` in the body. [-> UC-03](../usecases/UC10-encapsulation.md)
...
- [-> UC-01](../usecases/UC01-invalid-states.md) Union types are the foundation: `T | Null` is just `Union[T, Null]`.
- [-> UC-03](../usecases/UC10-encapsulation.md) Match types and pattern matching integrate with null narrowing.
- [-> UC-09](../usecases/UC16-nullability.md) Java interop patterns: wrapping legacy APIs that return nullable values with safe Scala facades.
- [-> UC-09](../usecases/UC16-nullability.md) Erased definitions combine with explicit nulls for fully safe null checking.
