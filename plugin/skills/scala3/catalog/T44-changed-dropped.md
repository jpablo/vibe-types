...
> **Since:** Scala 3.0 | **Latest changes:** Scala 3.7 (given disambiguation rule 9), Scala 3.6 (recursive given avoidance rule 10)

## What it is

Scala 3 deliberately changes several type-system behaviors inherited from Scala 2 and drops features that proved unsound or overly complex. **Type inference** is improved with better GADT support and a new algorithm. **Implicit resolution** uses a new search algorithm with refined disambiguation, proper ambiguity propagation, and prevention of recursive givens. **Existential types** (`forSome`) are removed entirely. **General type projection** (`T#A`) is restricted to class-type prefixes: an abstract type or type parameter can no longer be used as the prefix. Together these changes make the type system more predictable, sound, and amenable to tooling.
...
// Scala 2:  T#A where T is an abstract type  // unsound, no longer compiles
// Scala 3:  the prefix must be a class type (class or trait)
class Outer { class Inner }
type Valid = Outer#Inner            // ok: Outer is a class
trait Container { type Elem }
type AlsoValid = Container#Elem     // ok: trait prefix, abstract member projected
// type Invalid[T <: Container] = T#Elem   // "T is not a legal path" — T is abstract
```

## Interaction with other features

- **Explicit given types (rule 1).** Implicit vals and defs at class or object level must have an explicitly declared result type. This improves compilation speed (no need to infer the type before indexing) and prevents accidental implicit widening. Local implicit *vals* inside blocks are exempt; a local implicit `def` is not — `implicit def d = new C` inside a method still fails with "result type of implicit definition needs to be given explicitly".
...
- **Given disambiguation (rule 9).** From source version **3.7**, when multiple givens match an expected type, the _most general_ one is preferred (not the most specific, as in overloading resolution). Under `-source:3.5` the compiler still picks the most specific and says nothing; the change is announced by a warning under `-source:3.6`/`-source:3.6-migration` ("will change in the future release") and confirmed under `-source:3.7-migration` ("has changed"). (The scala-lang reference page dates this to 3.5; the compiler is the authority.)
- **Recursive given avoidance (rule 10).** Since source version **3.6** this is on by default — no flag needed — so it is already active on current compilers: implicit resolution discards search results that lead back to the given definition currently being checked. This is what makes `given Ordering[Price] = summon[Ordering[BigDecimal]]` in an opaque type companion resolve to `Ordering.BigDecimal` instead of looping. Under `-source:3.5` the same code is a hard error telling you to opt in with `-source 3.6` (or the corresponding `scala.language` import).
...
- **Class-type prefixes only for projection.** `T#A` is allowed when `T` is a class type — a trait or an abstract class counts, and the projected member may itself be abstract (`trait Container { type Elem }; type FromTrait = Container#Elem` compiles). What is rejected is an *abstract type* as the prefix: a type parameter or abstract type member (`type Bad[T <: Container] = T#Elem` fails with "T is not a legal path / since it is not a concrete type"). For those, use path-dependent types (`x.A` for some value `x: T`). [-> UC-04](../usecases/UC11-effect-tracking.md)
...
4. **Existential type approximation.** When reading Scala 2 classfiles containing existential types, Scala 3 performs a best-effort approximation silently — no warning is emitted, not even under `-Wall`. Some types may not round-trip precisely.
5. **Type projection restriction.** Dropping `T#A` for abstract `T` (type parameters and abstract type members) breaks type-level encoding tricks (e.g., the SKI combinator calculus encoding). The recommended alternative is path-dependent types, which require a value-level witness.
6. **Given preference reversal.** The shift from "most specific" to "most general" in given disambiguation (rule 9) can silently change which instance is selected. The flip happens at source version **3.7**, not 3.5 — `-source:3.5` and `-source:3.5-migration` still pick the most specific and warn about nothing. Compile with `-source:3.6-migration` (or plain `-source:3.6`) before the switch, and `-source:3.7-migration` after it, to see every site whose selection changes.
