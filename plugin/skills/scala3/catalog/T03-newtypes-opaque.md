...
| **Multiversal equality** [-> T20](T20-equality-safety.md) | `derives` is only allowed on a class, trait, object, or enum, so an opaque type declares its equality domain by hand: `given CanEqual[Meters, Meters] = CanEqual.derived` inside the defining object. That one given is enough to reject `Meters == Seconds` (`[E172] Values of types Units.Meters and Units.Seconds cannot be compared with == or !=`) *even without* `-language:strictEquality`; allowing the comparison would require an explicit `CanEqual[Meters, Seconds]`. |
...
- [-> UC-03](../usecases/UC10-encapsulation.md) Newtypes / zero-cost wrappers for domain primitives
- [-> UC-05](../usecases/UC12-compile-time.md) Units-of-measure without runtime overhead
- [-> UC-07](../usecases/UC14-extensibility.md) Encapsulation of internal representations in library APIs
- [-> UC-10](../usecases/UC17-variance.md) Permission systems with subtype-bounded opaque types
