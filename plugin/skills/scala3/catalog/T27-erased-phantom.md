...
- [-> UC-05](../usecases/UC12-compile-time.md) Type class evidence (e.g., `CanEqual`, `Ordering`) can be erased for zero-overhead constraints.
- [-> UC-09](../usecases/UC16-nullability.md) Explicit nulls: erased capabilities compose with null-safe types for comprehensive static checking.
- [-> UC-14](../usecases/UC08-error-handling.md) `CanThrow` capabilities for safer exceptions are the flagship use of erased definitions.
- [-> UC-06](../usecases/UC13-state-machines.md) Phantom types / state machines use erased evidence to enforce valid state transitions at zero runtime cost.
