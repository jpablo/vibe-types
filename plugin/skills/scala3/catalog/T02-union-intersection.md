...
- **Pattern matching.** Union types work naturally with `match` expressions for type narrowing. The compiler checks exhaustiveness over the union's members and *warns* (`[E029] Pattern Match Exhaustivity Warning`) when a case is missing. This fires for unions generally -- non-sealed case classes like `UserName | Password` above, and even `Int | String` -- but it is a warning, not an error, unless you compile with `-Werror`.
...
- **Type inference (union).** An inferred union is widened to its join only when its members share a common supertype that is neither a top type (`Any`, `AnyRef`, `Matchable`) nor `transparent`. Under `sealed trait Animal`, `if cond then Dog(...) else Cat(...)` widens to `Animal`. When no such supertype exists the inferred type *stays* the union: `if cond then 1 else "s"` infers `Int | String`, and `if cond then UserName(...) else Password(...)` infers `UserName | Password` (their only shared parents are `Any` plus the transparent `Product`/`Serializable`).
...
1. **An inferred union can be widened away.** Writing `val x = if cond then a else b` where `a: A` and `b: B` yields the join (least upper bound), not `A | B`, whenever `A` and `B` share a non-transparent, non-top supertype -- annotate explicitly if you wanted the union. If they do not, the union survives inference: `val z = if cond then 1 else "s"` is `Int | String`.
...
- [-> UC-05](../usecases/UC12-compile-time.md) Given instances can require combined evidence by stacking context bounds: `given [T: Ord : Show] => Ord[List[T]]` (or `[T: {Ord, Show}]`, Scala 3.6+). Note that `[T: Ord & Show]` is a *kind* error -- a context bound takes type constructors, not an intersection of them.
