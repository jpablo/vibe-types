...
- `[-> UC-01](../usecases/UC01-invalid-states.md)` Compile-time configuration and conditional compilation (logging, debug modes).
- `[-> UC-05](../usecases/UC12-compile-time.md)` Type-level arithmetic for sized vectors, matrix dimensions, or bounded naturals.
- `[-> UC-08](../usecases/UC15-equality.md)` Compile-time specialization of type class instances with `summonFrom`.
- `[-> UC-11](../usecases/UC18-type-arithmetic.md)` Recursive inline for unrolling loops or generating specialized code paths.
- `[-> UC-05](../usecases/UC12-compile-time.md)` Custom compile-time error messages for invalid type combinations.
- `[-> UC-05](../usecases/UC12-compile-time.md)` Entry point for macro definitions via `inline def` + `${ ... }`.
