...
3. **Invariant positions — scrutinee and patterns only.** The scrutinee and the case patterns are invariant positions, so a `+X`/`-X` parameter cannot appear there: `type Scrut[+X] = X match { case Int => String; case _ => Int }` is rejected with "covariant type parameter X occurs in invariant position", and so is `type Pat[+P] = Int match { case P => String; case _ => Int }`. Case **bodies**, however, are ordinary covariant positions — the match type is checked against the union of its bodies. `type Body[X, +Y] = X match { case Int => Y; case _ => Int }` compiles, while flipping the annotation to `-Y` fails with "contravariant type parameter Y occurs in covariant position". (The upstream Scala reference page still states that all three positions are invariant; the compiler is the authority here.)
