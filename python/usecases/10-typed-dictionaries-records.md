...
| Feature | Role | Link |
|---|---|---|
| `Literal` for key narrowing | Restrict dictionary keys to specific string values | [-> catalog/02](../catalog/02-union-literal-types.md) |
| `TypedDict` | Dict with per-key type annotations and required/optional control | [-> catalog/03](../catalog/03-typeddict.md) |
| `Annotated` on values | Carry validation metadata alongside value types | [-> catalog/15](../catalog/15-annotated-metadata.md) |
| `Unpack` | Pass TypedDict shapes as typed `**kwargs` | [-> catalog/19](../catalog/19-unpack-kwargs-typing.md) |
