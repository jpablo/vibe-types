...
| Feature | Role | Link |
|---|---|---|
| Generics (`TypeVar`) | Preserve a concrete type through a generic wrapper | [-> catalog/07](../catalog/07-generics-typevar.md) |
| `ParamSpec` | Capture and forward an entire parameter signature | [-> catalog/08](../catalog/08-paramspec-typevar-tuple.md) |
| Protocol `__call__` | Describe complex callable shapes structurally | [-> catalog/09](../catalog/09-protocol-structural-subtyping.md) |
| `Callable` / `@overload` | Annotate simple callbacks and multi-signature APIs | [-> catalog/11](../catalog/11-callable-types-overload.md) |
| `Self` | Return the same type from fluent/chained methods | [-> catalog/16](../catalog/16-self-type.md) |
