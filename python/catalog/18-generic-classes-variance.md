...
|---------|-----------------|
| **TypeVar** [-> catalog/07](07-generics-typevar.md) | Pre-3.12, variance is declared on the TypeVar: `T_co = TypeVar("T_co", covariant=True)`. The 3.12 syntax removes this manual step. |
| **ParamSpec / TypeVarTuple** [-> catalog/08](08-paramspec-typevar-tuple.md) | ParamSpec and TypeVarTuple follow their own variance rules. ParamSpec is typically invariant. TypeVarTuple elements are individually covariant in `Unpack` positions for read-only containers. |
| **Protocol** [-> catalog/09](09-protocol-structural-subtyping.md) | Protocols can be generic and are variance-checked like regular generics. A Protocol with `T` only in return positions is covariant. |
| **Callable** [-> catalog/11](11-callable-types-overload.md) | `Callable[[T], R]` is contravariant in `T` (input) and covariant in `R` (output). This is the classic example of mixed variance. |
...
- [-> UC-04](../usecases/04-generic-constraints.md) — Designing generic container APIs with correct variance for type-safe collection hierarchies.
- [-> UC-11](../usecases/11-decorator-typing.md) — Event systems and callback registries where contravariant handlers enable safe substitution.
