...
|---------|-----------------|
| **Basic annotations** [-> catalog/01](01-basic-annotations-none.md) | Annotations are the entry point for the type system. Without them, the checker falls back to inference or `Any`. |
| **TypeAlias** [-> catalog/17](17-type-aliases-type-statement.md) | Type aliases interact with inference: a bare `X = int` may be inferred as a variable rather than an alias if `TypeAlias` is not used. |
| **Annotated** [-> catalog/15](15-annotated-metadata.md) | `Annotated[T, ...]` carries metadata alongside an inferred or explicit type. The base type `T` participates in inference normally. |
| **Protocol** [-> catalog/09](09-protocol-structural-subtyping.md) | Protocols enable structural typing, which works with inference: if an object has the right methods, it satisfies the Protocol even without explicit annotation. |
...
- [-> UC-12](../usecases/12-gradual-adoption.md) — Gradual typing adoption strategies for legacy codebases and library boundaries.
